...
const LEADING_NOISE = /^(?:كتاب|رواية|قصة|ديوان|كتب|روايات|تحميل|تنزيل|حمّل|نزّل|اقرأ|قراءة|لخصلي|لخّصلي|لخّص\s+لي|لخص\s+لي|لخّص|لخص|ملخص|ملخّص|مُلخّص|تلخيص|اختصرلي|اختصر\s+لي|اختصر|عايز|عاوز|عايزة|أريد|اريد|ممكن|عندك|جبلي|جيبلي|ابعتلي|هاتلي|ابحث(?:\s*لي)?(?:\s*عن)?|دور(?:\s*لي)?(?:\s*على)?|لو\s*سمحت|من\s*فضلك)\s+/i;
